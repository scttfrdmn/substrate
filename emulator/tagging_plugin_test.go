...
func newTaggingTestServer(t *testing.T) (*httptest.Server, emulator.StateManager) {
	t.Helper()
	state := emulator.NewMemoryStateManager()
	logger := emulator.NewDefaultLogger(slog.LevelError, false)
	registry := emulator.NewPluginRegistry()
	cfg := emulator.Config{}
	store := emulator.NewEventStore(emulator.EventStoreConfig{Enabled: false, Backend: "memory"})
	tc := emulator.NewTimeController(time.Now())
	initCtx := context.Background()

	iamPlugin := &emulator.IAMPlugin{}
	require.NoError(t, iamPlugin.Initialize(initCtx, emulator.PluginConfig{State: state, Logger: logger}))
	registry.Register(iamPlugin)

	lambdaPlugin := &emulator.LambdaPlugin{}
	require.NoError(t, lambdaPlugin.Initialize(initCtx, emulator.PluginConfig{
	...
	require.NoError(t, sqsPlugin.Initialize(initCtx, emulator.PluginConfig{
		State: state, Logger: logger,
		Options: map[string]any{"time_controller": tc},
	}))
	...
	require.NoError(t, dynamodbPlugin.Initialize(initCtx, emulator.PluginConfig{
		State: state, Logger: logger,
		Options: map[string]any{"time_controller": tc},
	}))
	...
	require.NoError(t, ec2Plugin.Initialize(initCtx, emulator.PluginConfig{
		State: state, Logger: logger,
		Options: map[string]any{"time_controller": tc},
	}))
	...
	require.NoError(t, s3Plugin.Initialize(initCtx, emulator.PluginConfig{
		State: state, Logger: logger,
...
func putTestS3Bucket(t *testing.T, state emulator.StateManager, name string, tags map[string]string) {
	t.Helper()
...
func putTestLambdaFunction(t *testing.T, state emulator.StateManager, name string, tags map[string]string) {
	t.Helper()
...
func putTestEC2Instance(t *testing.T, state emulator.StateManager, id string, tags []emulator.EC2Tag) {
	t.Helper()
...
func TestTagging_GetResources_EC2(t *testing.T) {
	ts, state := newTaggingTestServer(t)
	putTestEC2Instance(t, state, "i-abc123", []emulator.EC2Tag{{Key: "Name", Value: "test-vm"}})
...
func putTestECSCluster(t *testing.T, state emulator.StateManager, name string, tags []emulator.ECSTag) {
	t.Helper()
	arn := "arn:aws:ecs:us-east-1:" + taggingTestAccountID + ":cluster/" + name
	cluster := emulator.ECSCluster{
		ClusterArn:  arn,
		ClusterName: name,
		Status:      "ACTIVE",
		Tags:        tags,
...
func putTestECRRepository(t *testing.T, state emulator.StateManager, name string, tags map[string]string) {
	t.Helper()
	arn := "arn:aws:ecr:us-east-1:" + taggingTestAccountID + ":repository/" + name
	repo := emulator.ECRRepository{
		RepositoryName: name,
		RepositoryArn:  arn,
...
func putTestStateMachine(t *testing.T, state emulator.StateManager, name string, tags map[string]string) {
	t.Helper()
	arn := "arn:aws:states:us-east-1:" + taggingTestAccountID + ":stateMachine:" + name
	sm := emulator.StateMachineState{
		StateMachineArn: arn,
		Name:            name,
		Status:          "ACTIVE",
		Tags:            tags,
...
func putTestKinesisStream(t *testing.T, state emulator.StateManager, name string, tags map[string]string) {
	t.Helper()
	arn := "arn:aws:kinesis:us-east-1:" + taggingTestAccountID + ":stream/" + name
	stream := emulator.KinesisStream{
		StreamName:   name,
		StreamArn:    arn,
		StreamStatus: "ACTIVE",
		Tags:         tags,
...
func putTestCognitoUserPool(t *testing.T, state emulator.StateManager, poolID string, tags map[string]string) {
	t.Helper()
	arn := "arn:aws:cognito-idp:us-east-1:" + taggingTestAccountID + ":userpool/" + poolID
	pool := emulator.CognitoUserPool{
		UserPoolID: poolID,
		Name:       "test-pool",
		Arn:        arn,
		Status:     "Enabled",
...
func TestTagging_GetResources_ECSCluster(t *testing.T) {
	ts, state := newTaggingTestServer(t)
	putTestECSCluster(t, state, "my-cluster", []emulator.ECSTag{{Key: "Env", Value: "prod"}})
...
func TestTagging_TagResources_ECSCluster(t *testing.T) {
	...
	require.NoError(t, err)
	var cluster emulator.ECSCluster
...
func TestTagging_TagResources_ECRRepository(t *testing.T) {
	...
	require.NoError(t, err)
	var repo emulator.ECRRepository
...
func TestTagging_TagResources_StateMachine(t *testing.T) {
	...
	require.NoError(t, err)
	var sm emulator.StateMachineState
...
func TestTagging_TagResources_KinesisStream(t *testing.T) {
	...
	require.NoError(t, err)
	var stream emulator.KinesisStream
...
func TestTagging_TagResources_CognitoUserPool(t *testing.T) {
	...
	require.NoError(t, err)
	var pool emulator.CognitoUserPool
	require.NoError(t, json.Unmarshal(raw, &pool))
	assert.Equal(t, "us-east-1", pool.Tags["Region"])
}

func putTestEFSFileSystem(t *testing.T, state emulator.StateManager, fsID string, tags []emulator.EFSTag) {
	t.Helper()
	fs := emulator.EFSFileSystem{
		FileSystemID:   fsID,
...
func putTestGlueDatabase(t *testing.T, state emulator.StateManager, name string, tags map[string]string) {
	t.Helper()
	db := emulator.GlueDatabase{
		Name:      name,
...
func TestTagging_GetResources_EFSFileSystem(t *testing.T) {
	ts, state := newTaggingTestServer(t)
	putTestEFSFileSystem(t, state, "fs-abc12345", []emulator.EFSTag{{Key: "Name", Value: "my-fs"}})
