...
func TestEventStore_WithTimeController(t *testing.T) {
	...
	tc := emulator.NewTimeController(fixedTime)

	store := emulator.NewEventStore(
		emulator.EventStoreConfig{Enabled: true, Backend: "memory"},
...
func TestEventStore_RecordEvent_Disabled(t *testing.T) {
	...
	event := &emulator.Event{Service: "s3", Operation: "PutObject", Timestamp: time.Now()}
	require.NoError(t, store.RecordEvent(ctx, event))
...
func TestEventStore_ReplayIterator(t *testing.T) {
	...
	iter, err := store.NewReplayIterator(ctx, emulator.EventFilter{StreamID: "s"})
	require.NoError(t, err)
...
func TestJSONSerializer(t *testing.T) {
	s := &emulator.JSONSerializer{}

	original := &emulator.Event{
		ID:        "evt-1",
		Sequence:  42,
		Service:   "s3",
		Operation: "PutObject",
...
func TestEventStore_SnapshotInterval_Zero_Disabled(t *testing.T) {
	...
	sm := emulator.NewMemoryStateManager()
	store := emulator.NewEventStore(
		emulator.EventStoreConfig{Enabled: true, Backend: "memory", SnapshotInterval: 0},
		emulator.WithStateManager(sm),
	)
	ctx := context.Background()
	for range 10 {
		require.NoError(t, store.RecordEvent(ctx, &emulator.Event{
...
func TestEventStore_AsyncSnapshot_CreatesSnapshot(t *testing.T) {
	sm := emulator.NewMemoryStateManager()
	store := emulator.NewEventStore(
		emulator.EventStoreConfig{Enabled: true, Backend: "memory", SnapshotInterval: 5},
		emulator.WithStateManager(sm),
	)
...
func TestEventStore_FilePersistence_AppendOnly(t *testing.T) {
	...
	events, err := store2.GetEvents(ctx, emulator.EventFilter{})
	require.NoError(t, err)
...
func TestExportNDJSON_Empty(t *testing.T) {
	...
	n, err := store.ExportNDJSON(ctx, emulator.EventFilter{}, &buf)
	require.NoError(t, err)
...
func TestExportCSV_HeaderRow(t *testing.T) {
	...
	ctx := context.Background()
	var buf bytes.Buffer
	n, err := store.ExportCSV(ctx, emulator.EventFilter{}, &buf)
	require.NoError(t, err)
	assert.Equal(t, int64(0), n)
