...
func newCEWithEC2TestServer(t *testing.T) (*httptest.Server, *emulator.TimeController) {
	t.Helper()
	registry := emulator.NewPluginRegistry()
	store := emulator.NewEventStore(emulator.EventStoreConfig{Enabled: true, Backend: "memory"})
	state := emulator.NewMemoryStateManager()
	...
	if err := ec2p.Initialize(t.Context(), emulator.PluginConfig{ //nolint:contextcheck
		State:   state,
		Logger:  logger,
	...
	cep := &emulator.CEPlugin{}
	if err := cep.Initialize(t.Context(), emulator.PluginConfig{ //nolint:contextcheck
		State:  state,
		Logger: logger,
		Options: map[string]any{
			"event_store":     store,
