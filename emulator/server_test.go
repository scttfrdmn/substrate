...
// serverPlugin is a configurable [emulator.Plugin] used only by server tests.
...
func (p *serverPlugin) Initialize(_ context.Context, _ emulator.PluginConfig) error { return nil }

func (p *serverPlugin) HandleRequest(_ *emulator.RequestContext, _ *emulator.AWSRequest) (*emulator.AWSResponse, error) {
	return p.resp, p.err
}
...
func newTestServer(t *testing.T, plugins ...emulator.Plugin) *emulator.Server {
	t.Helper()
...
func TestServer_ReadyEndpoint(t *testing.T) {
	plug := &serverPlugin{serviceName: "s3", resp: &emulator.AWSResponse{StatusCode: 200}}
	srv := newTestServer(t, plug)
...
func TestServer_MetricsEndpoint(t *testing.T) {
	...
	tc := emulator.NewTimeController(time.Now())
	logger := emulator.NewDefaultLogger(slog.LevelInfo, false)
	metrics := emulator.NewMetricsCollector()
	srv := emulator.NewServer(*cfg, registry, store, state, tc, logger,
		emulator.ServerOptions{Metrics: metrics})
...
func TestServer_TracerSpanAttributes(t *testing.T) {
	...
	tracer, shutdown, err := emulator.NewTracer(context.Background(), emulator.TracingConfig{
		Enabled:     true,
		Exporter:    "noop",
		ServiceName: "test",
	})
	...
	logger := emulator.NewDefaultLogger(slog.LevelInfo, false)
	srv := emulator.NewServer(*cfg, registry, store, state, tc, logger,
		emulator.ServerOptions{Tracer: tracer})
...
func TestServer_EmailsEndpoint(t *testing.T) {
	...
	sesv2 := &emulator.SESv2Plugin{}
	require.NoError(t, sesv2.Initialize(context.Background(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
