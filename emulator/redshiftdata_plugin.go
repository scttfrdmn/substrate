...
// generateRedshiftDataID mints a statement ID from m.
//
// This is the one identifier in the analytics family whose shape AWS states outright:
// API_ExecuteStatement documents `Id` as "a universally unique identifier (UUID) generated by Amazon
// Redshift Data API" and publishes the pattern `[a-z0-9]{8}(-[a-z0-9]{4}){3}-[a-z0-9]{12}(:\d{0,2})?`.
// The character class is `[a-z0-9]` rather than `[0-9a-f]`, and it constrains no position, so it is
// indifferent to the RFC 4122 version and variant nibbles — which is why [IDMint.HexUUID] satisfies it
// and the rendering the crypto/rand form produced is preserved rather than changed (#671).
//
// Substrate mints no session, so the optional `:\d{0,2}` suffix — which the same pattern admits on
// `SessionId` — is never emitted.
//
// Deriving it is what lets a recorded statement be read back: `DescribeStatement`, `GetStatementResult`
// and `CancelStatement` all address the statement by this id, so a re-minted one answered each with
// ResourceNotFoundException against a statement the recording had just submitted.
func generateRedshiftDataID(m *IDMint) string {
	return m.HexUUID()
}
