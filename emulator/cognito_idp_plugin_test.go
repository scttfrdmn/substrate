...
func setupCognitoIDPPlugin(t *testing.T) (*emulator.CognitoIDPPlugin, *emulator.RequestContext) {
	t.Helper()
	...
	if err := p.Initialize(context.Background(), emulator.PluginConfig{
		...
		t.Fatalf("emulator.CognitoIDPPlugin.Initialize: %v", err)
	}
	return p, &emulator.RequestContext{AccountID: "123456789012", Region: "us-east-1", RequestID: "req-1"}
}

func setupCognitoIdentityPlugin(t *testing.T) (*emulator.CognitoIdentityPlugin, *emulator.RequestContext) {
	t.Helper()
	state := emulator.NewMemoryStateManager()
	tc := emulator.NewTimeController(time.Now())
	...
	if err := p.Initialize(context.Background(), emulator.PluginConfig{
		...
		t.Fatalf("emulator.CognitoIdentityPlugin.Initialize: %v", err)
	}
	return p, &emulator.RequestContext{AccountID: "123456789012", Region: "us-east-1", RequestID: "req-1"}
}

func cognitoIDPRequest(t *testing.T, op string, body map[string]any) *emulator.AWSRequest {
	t.Helper()
...
func cognitoIdentityRequest(t *testing.T, op string, body map[string]any) *emulator.AWSRequest {
	t.Helper()
	b, err := json.Marshal(body)
	if err != nil {
...
func createCognitoPool(t *testing.T, p *emulator.CognitoIDPPlugin, ctx *emulator.RequestContext, name string) string {
	t.Helper()
