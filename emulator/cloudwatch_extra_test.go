...
func TestCWLogs_Shutdown(t *testing.T) {
	p := &emulator.CloudWatchLogsPlugin{}
...
func TestLambda_AutoCreatesLogGroup(t *testing.T) {
	...
	require.NoError(t, lambdaPlugin.Initialize(context.TODO(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
	...
	require.NoError(t, cwLogsPlugin.Initialize(context.TODO(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
		Options: map[string]any{"time_controller": tc},
	}))
