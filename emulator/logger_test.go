...
func newTestLogger(buf *bytes.Buffer, level slog.Level) emulator.Logger {
	handler := slog.NewTextHandler(buf, &slog.HandlerOptions{Level: level})
	return emulator.NewSlogLogger(slog.New(handler))
}
...
func TestSlogLogger_Levels(t *testing.T) {
	...
	cases := []struct {
		name  string
		level slog.Level
		log   func(emulator.Logger)
		want  string
	}{
		{
			name:  "debug",
			level: slog.LevelDebug,
			log:   func(l emulator.Logger) { l.Debug("debug message", "k", "v") },
			want:  "debug message",
		},
		{
			name:  "info",
			level: slog.LevelInfo,
			log:   func(l emulator.Logger) { l.Info("info message", "k", "v") },
			want:  "info message",
		},
		{
			name:  "warn",
			level: slog.LevelInfo,
			log:   func(l emulator.Logger) { l.Warn("warn message", "k", "v") },
			want:  "warn message",
		},
		{
			name:  "error",
			level: slog.LevelInfo,
			log:   func(l emulator.Logger) { l.Error("error message", "k", "v") },
			want:  "error message",
		},
	}
...
func TestNewDefaultLogger_TextFormat(t *testing.T) {
	...
	l := emulator.NewDefaultLogger(slog.LevelInfo, false)
	if l == nil {
...
func TestNewDefaultLogger_JSONFormat(t *testing.T) {
	t.Parallel()
	l := emulator.NewDefaultLogger(slog.LevelInfo, true)
	if l == nil {
...
func TestSlogLogger_ImplementsInterface(t *testing.T) {
	t.Parallel()
	_ = emulator.NewSlogLogger(slog.Default())
}
