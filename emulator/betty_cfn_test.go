...
func newTestDeployer(t *testing.T) *emulator.StackDeployer {
	...
	appSyncPlugin := &emulator.AppSyncPlugin{}
	require.NoError(t, appSyncPlugin.Initialize(context.Background(), emulator.PluginConfig{
		State:  state,
		Logger: logger,
		Options: map[string]any{
			"time_controller": tc,
...
func newEC2TestDeployer(t *testing.T) *emulator.StackDeployer {
	t.Helper()
	cfg := emulator.DefaultConfig()
	registry := emulator.NewPluginRegistry()
	state := emulator.NewMemoryStateManager()
...
func TestCFN_RDSDBCluster(t *testing.T) {
	...
	var clusterResource emulator.DeployedResource
	for _, r := range result.Resources {
...
func newTestDeployerWithState(t *testing.T) (*emulator.StackDeployer, *emulator.MemoryStateManager) {
	...
	iamPlugin := &emulator.IAMPlugin{}
	require.NoError(t, iamPlugin.Initialize(context.Background(), emulator.PluginConfig{State: state, Logger: logger}))
	registry.Register(iamPlugin)
	...
	dynamoPlugin := &emulator.DynamoDBPlugin{}
	require.NoError(t, dynamoPlugin.Initialize(context.Background(), emulator.PluginConfig{
