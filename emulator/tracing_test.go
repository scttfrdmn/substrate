...
func TestNewTracer_UnknownExporter(t *testing.T) {
	_, _, err := emulator.NewTracer(context.Background(), emulator.TracingConfig{
		Enabled:  true,
...
func TestRecordSpanError_NilSpan(_ *testing.T) {
	...
	emulator.RecordSpanError(nil, fmt.Errorf("test error"))
}
...
func TestRecordSpanError_NoopSpan(t *testing.T) {
	...
	emulator.RecordSpanError(span, fmt.Errorf("NoSuchKey"))
	span.End()
}
