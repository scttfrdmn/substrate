...
func newSQSTestServer(t *testing.T) (*emulator.Server, *emulator.TimeController) {
	...
	return emulator.NewServer(*cfg, registry, store, state, tc, logger), tc
}

func sqsRequest(t *testing.T, srv *emulator.Server, params map[string]string) *http.Response {
	t.Helper()
	form := url.Values{}
...
func sqsJSONRequest(t *testing.T, srv *emulator.Server, target string, body interface{}) *http.Response {
	t.Helper()
