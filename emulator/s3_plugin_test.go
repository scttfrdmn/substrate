...
func s3Request(t *testing.T, srv *emulator.Server, method, path string, body []byte, headers map[string]string) *httptest.ResponseRecorder {
	t.Helper()
...
func s3VirtualHostRequest(t *testing.T, srv *emulator.Server, method, bucket, keyPath string, body []byte, headers map[string]string) *httptest.ResponseRecorder {
	t.Helper()
...
func newS3SQSTestServer(t *testing.T) *emulator.Server {
	t.Helper()
	cfg := emulator.DefaultConfig()
	state := emulator.NewMemoryStateManager()
	logger := emulator.NewDefaultLogger(slog.LevelError, false)
	...
	sqsp := &emulator.SQSPlugin{}
	require.NoError(t, sqsp.Initialize(context.Background(), emulator.PluginConfig{
		State:  state,
		Logger: logger,
		Options: map[string]any{
			"time_controller": tc,
...
func sqsFormRequest(t *testing.T, srv *emulator.Server, params map[string]string) *httptest.ResponseRecorder {
	t.Helper()
