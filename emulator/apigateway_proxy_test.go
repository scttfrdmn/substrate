...
func TestBuildV1ProxyEvent(t *testing.T) {
	...
	data, err := emulator.BuildV1ProxyEventForTest(req, "abc123", "prod", "/users")
	if err != nil {
...
func TestParseV1ProxyResponse(t *testing.T) {
	...
	got, err := emulator.ParseProxyResponseForTest(respBody)
	if err != nil {
...
func TestBuildV2ProxyEvent(t *testing.T) {
	...
	data, err := emulator.BuildV2ProxyEventForTest(req, "xyz789", "prod", "/orders")
	if err != nil {
...
func TestExtractLambdaARNFromURI(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.uri[:min(len(tc.uri), 40)], func(t *testing.T) {
			t.Parallel()
			got := emulator.ExtractLambdaARNFromURIForTest(tc.uri)
			if got != tc.want {
...
func TestParseProxyResponse_InvalidJSON(t *testing.T) {
	...
	got, err := emulator.ParseProxyResponseForTest(raw)
	if err != nil {
		t.Fatalf("ParseProxyResponse: %v", err)
	}
...
func proxyDoRequest(t *testing.T, ts *emulator.TestServer, method, path, host, contentType string, body io.Reader) *http.Response {
	t.Helper()
...
func proxyCreateLambdaFunction(t *testing.T, ts *emulator.TestServer, name string) {
	t.Helper()
...
func proxyCreateRESTAPI(t *testing.T, ts *emulator.TestServer) string {
	t.Helper()
	payload, _ := json.Marshal(map[string]interface{}{
...
func proxyGetRootResourceID(t *testing.T, ts *emulator.TestServer, apiID string) string {
	t.Helper()
...
func proxyPutMethod(t *testing.T, ts *emulator.TestServer, apiID, resourceID, httpMethod string) {
	t.Helper()
	payload, _ := json.Marshal(map[string]interface{}{
...
func proxyCreateIntegration(t *testing.T, ts *emulator.TestServer, apiID, resourceID, httpMethod, uri string) {
	t.Helper()
	payload, _ := json.Marshal(map[string]interface{}{
		"type":       "AWS_PROXY",
		"httpMethod": "POST",
