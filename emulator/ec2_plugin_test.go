...
func TestEC2_SecurityGroupAllowed(t *testing.T) {
	rules := []emulator.EC2IPPermission{
	...
	assert.True(t, emulator.SecurityGroupAllowed(rules, "tcp", 80, "192.168.1.1"))

	// TCP 443 from 10.0.1.1 (within 10.0.0.0/8) → allowed.
	assert.True(t, emulator.SecurityGroupAllowed(rules, "tcp", 443, "10.0.1.1"))

	// TCP 443 from 192.168.1.1 (not in 10.0.0.0/8) → denied.
	assert.False(t, emulator.SecurityGroupAllowed(rules, "tcp", 443, "192.168.1.1"))

	// TCP 22 from anywhere → denied (no rule for port 22).
	assert.False(t, emulator.SecurityGroupAllowed(rules, "tcp", 22, "0.0.0.0"))

	// UDP 80 → denied (wrong protocol).
	assert.False(t, emulator.SecurityGroupAllowed(rules, "udp", 80, "0.0.0.0"))

	// All-traffic rule.
	allRules := []emulator.EC2IPPermission{
		{IPProtocol: "-1", IPRanges: []string{"0.0.0.0/0"}},
	}
	assert.True(t, emulator.SecurityGroupAllowed(allRules, "tcp", 22, "1.2.3.4"))
	assert.True(t, emulator.SecurityGroupAllowed(allRules, "udp", 53, "10.0.0.1"))
}
