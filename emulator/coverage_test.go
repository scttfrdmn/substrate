...
func TestPluginShutdown_NoopPlugins(t *testing.T) {
	...
	plugins := []emulator.Plugin{
		&emulator.AthenaPlugin{},
		&emulator.BackupPlugin{},
		&emulator.BatchPlugin{},
		&emulator.BedrockRuntimePlugin{},
		&emulator.CloudTrailPlugin{},
		&emulator.CodeBuildPlugin{},
		&emulator.CodeDeployPlugin{},
		&emulator.CodePipelinePlugin{},
		&emulator.EMRServerlessPlugin{},
		&emulator.FSxPlugin{},
		&emulator.OmicsPlugin{},
		&emulator.OpenSearchPlugin{},
		&emulator.QuickSightPlugin{},
		&emulator.RAMPlugin{},
		&emulator.RedshiftPlugin{},
		&emulator.RedshiftDataPlugin{},
		&emulator.SageMakerPlugin{},
		&emulator.TransferPlugin{},
		&emulator.WAFv2Plugin{},
		&emulator.SchedulerPlugin{},
		&emulator.SSOPlugin{},
	}
...
func (p *mockPlugin) Initialize(_ context.Context, _ emulator.PluginConfig) error { return nil }
func (p *mockPlugin) HandleRequest(_ *emulator.RequestContext, _ *emulator.AWSRequest) (*emulator.AWSResponse, error) {
	return &emulator.AWSResponse{StatusCode: 200}, nil
}
...
func TestEventStore_RecordRequest(t *testing.T) {
	store := emulator.NewEventStore(emulator.EventStoreConfig{Enabled: true, Backend: "memory", IncludeBodies: true})
	ctx := context.Background()

	reqCtx := &emulator.RequestContext{
		AccountID: "123456789012",
		Region:    "us-east-1",
		Principal: &emulator.Principal{ARN: "arn:aws:iam::123:user/alice"},
		Metadata:  map[string]interface{}{"stream_id": "stream-rr"},
	}
	req := &emulator.AWSRequest{Service: "s3", Operation: "PutObject"}
	resp := &emulator.AWSResponse{StatusCode: 200}
...
func TestEventStore_RecordRequest_WithError(t *testing.T) {
	...
	// Plain error.
	plainErr := &emulator.AWSError{Code: "AccessDenied", Message: "denied"}
	err := store.RecordRequest(ctx, reqCtx, req, nil, 0, 0, plainErr)
	require.NoError(t, err)

	events, err := store.GetEvents(ctx, emulator.EventFilter{Service: "s3"})
	require.NoError(t, err)
	require.Len(t, events, 1)
...
func TestEventStore_RecordRequest_NoBodies(t *testing.T) {
	store := emulator.NewEventStore(emulator.EventStoreConfig{Enabled: true, Backend: "memory", IncludeBodies: false})
	ctx := context.Background()

	req := &emulator.AWSRequest{Service: "s3", Operation: "GetObject"}
	resp := &emulator.AWSResponse{StatusCode: 200}
	err := store.RecordRequest(ctx, &emulator.RequestContext{}, req, resp, 0, 0, nil)
	require.NoError(t, err)
...
func TestEventFilter_HasError(t *testing.T) {
	...
	require.NoError(t, store.RecordEvent(ctx, &emulator.Event{Service: "s3", Timestamp: time.Now()}))
	require.NoError(t, store.RecordEvent(ctx, &emulator.Event{Service: "s3", Error: "boom", Timestamp: time.Now()}))

	errOnly, err := store.GetEvents(ctx, emulator.EventFilter{HasError: &hasErr})
	require.NoError(t, err)
	assert.Len(t, errOnly, 1)

	noErrOnly, err := store.GetEvents(ctx, emulator.EventFilter{HasError: &noErr})
	require.NoError(t, err)
...
func TestEventFilter_SequenceRange(t *testing.T) {
	...
	events, err := store.GetEvents(ctx, emulator.EventFilter{MinSequence: 1, MaxSequence: 3})
	require.NoError(t, err)
...
func newEngineWithEvents(t *testing.T, n int) (*emulator.ReplayEngine, string) {
	t.Helper()
...
func TestReplayEngine_Replay_WithRequestAndUnknownService(t *testing.T) {
	...
	require.NoError(t, store.RecordEvent(ctx, &emulator.Event{
		...
		Operation: "GetItem",
		Timestamp: time.Now(),
		Request:   &emulator.AWSRequest{Service: "dynamodb", Operation: "GetItem"},
		Response:  &emulator.AWSResponse{StatusCode: 200},
	}))
	...
	engine := emulator.NewReplayEngine(store, nil, tc, registry,
		emulator.ReplayConfig{StopOnError: true}, &testLogger{})
...
func TestReplayEngine_Replay_StopOnError(t *testing.T) {
	...
	for range 2 {
		require.NoError(t, store.RecordEvent(ctx, &emulator.Event{
			...
			Timestamp: time.Now(),
			Request:   &emulator.AWSRequest{Service: "sqs", Operation: "SendMessage"},
		}))
	}
...
func TestReplayEngine_Replay_DifferentStatusCode(t *testing.T) {
	...
	require.NoError(t, store.RecordEvent(ctx, &emulator.Event{
		...
		Operation: "GetObject",
		Timestamp: time.Now(),
		Request:   &emulator.AWSRequest{Service: "s3", Operation: "GetObject"},
		Response:  &emulator.AWSResponse{StatusCode: 404}, // different from what plugin returns
	}))
...
func TestIAMPlugin_Authorize_WithInlinePolicy(t *testing.T) {
	...
	reqCtx := &emulator.RequestContext{
		AccountID: "123456789012",
		Principal: &emulator.Principal{ARN: "arn:aws:iam::123456789012:user/inlined", Type: "IAMUser"},
		Metadata:  make(map[string]interface{}),
	}

	// Should allow s3:GetObject via inline policy.
	err := emulator.IAMAuthorizeForTest(plugin, reqCtx, "s3:GetObject", "*")
	assert.NoError(t, err)

	// Should deny s3:PutObject (not in inline policy).
	err = emulator.IAMAuthorizeForTest(plugin, reqCtx, "s3:PutObject", "*")
	require.Error(t, err)
}
...
func TestIAMPlugin_Authorize_WithBoundary(t *testing.T) {
	...
	boundaryPolicy := emulator.IAMPolicy{
	...
	reqCtx := &emulator.RequestContext{
		AccountID: "123456789012",
		Principal: &emulator.Principal{ARN: "arn:aws:iam::123456789012:user/bounded", Type: "IAMUser"},
		Metadata:  make(map[string]interface{}),
	}
