...
func newDynamoDBTestServer(t *testing.T) *emulator.Server {
	t.Helper()
	...
	require.NoError(t, dynamoPlugin.Initialize(context.TODO(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
...
func dynamodbRequest(t *testing.T, srv *emulator.Server, op string, body any) *http.Response {
	t.Helper()
...
func TestCFN_DynamoDB(t *testing.T) {
	...
	dynamoSrv := emulator.NewServer(*cfg, registry, store, state, tc, logger)
...
func TestDynamoDB_ShutdownAndName(t *testing.T) {
	...
	plugin := &emulator.DynamoDBPlugin{}
	state := emulator.NewMemoryStateManager()
	logger := emulator.NewDefaultLogger(0, false)
	require.NoError(t, plugin.Initialize(t.Context(), emulator.PluginConfig{State: state, Logger: logger}))
