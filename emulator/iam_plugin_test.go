...
func iamRequest(t *testing.T, srv *emulator.Server, operation string, body any) *http.Response {
	t.Helper()
	var bodyBytes []byte
	if body != nil {
		var err error
...
func TestIAMPlugin_Shutdown(t *testing.T) {
	...
	state := emulator.NewMemoryStateManager()
	require.NoError(t, plugin.Initialize(context.TODO(), emulator.PluginConfig{State: state, Logger: logger}))
	require.NoError(t, plugin.Shutdown(context.TODO()))
}

func TestSTSPlugin_Shutdown(t *testing.T) {
	plugin := &emulator.STSPlugin{}
	logger := emulator.NewDefaultLogger(0, false)
	state := emulator.NewMemoryStateManager()
	require.NoError(t, plugin.Initialize(context.TODO(), emulator.PluginConfig{State: state, Logger: logger}))
	require.NoError(t, plugin.Shutdown(context.TODO()))
