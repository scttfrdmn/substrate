...
func setupSESv2Plugin(t *testing.T) (*emulator.SESv2Plugin, *emulator.RequestContext) {
	t.Helper()
...
func sesv2Request(method, path string, body map[string]any) *emulator.AWSRequest {
	var b []byte
	if body != nil {
...
func TestSESv2Plugin(t *testing.T) {
	...
	t.Run("SendEmail_Capture", func(t *testing.T) {
		...
		state := emulator.NewMemoryStateManager()
		p3 := &emulator.SESv2Plugin{}
		_ = p3.Initialize(context.Background(), emulator.PluginConfig{
			State:  state,
			Logger: emulator.NewDefaultLogger(slog.LevelError, false),
		})
		_ = p3
