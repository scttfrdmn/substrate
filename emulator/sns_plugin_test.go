...
func newSNSTestServer(t *testing.T) *emulator.Server {
	t.Helper()
	...
	require.NoError(t, sqsPlugin.Initialize(context.TODO(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
	...
	require.NoError(t, snsPlugin.Initialize(context.TODO(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
...
func snsRequest(t *testing.T, srv *emulator.Server, params map[string]string) *http.Response {
	t.Helper()
	form := url.Values{}
...
func sqsRequestViaSNSServer(t *testing.T, srv *emulator.Server, params map[string]string) *http.Response {
	t.Helper()
	form := url.Values{}
	for k, v := range params {
		form.Set(k, v)
