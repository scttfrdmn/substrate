...
func makeQuotaReqCtx() *emulator.RequestContext {
...
func TestQuotaController_Disabled_AllowsUnlimited(t *testing.T) {
	...
	ctrl := emulator.NewQuotaController(emulator.QuotaConfig{Enabled: false}, tc)
...
func TestQuotaController_BurstExhausted_Returns429(t *testing.T) {
	tc := emulator.NewTimeController(time.Now())
	cfg := emulator.QuotaConfig{
		Enabled: true,
		Rules: map[string]emulator.RateRule{
			"iam": {Rate: 1, Burst: 5},
		},
	}
	ctrl := emulator.NewQuotaController(cfg, tc)

	req := &emulator.AWSRequest{Service: "iam", Operation: "ListUsers"}
	reqCtx := makeQuotaReqCtx()
...
func TestQuotaController_OperationKeyTakesPrecedence(t *testing.T) {
	...
	createReq := &emulator.AWSRequest{Service: "iam", Operation: "CreateUser"}
	...
	listReq := &emulator.AWSRequest{Service: "iam", Operation: "ListUsers"}
	for i := 0; i < 10; i++ {
...
func TestQuotaController_Integration_Returns429XMLBody(t *testing.T) {
	...
	quotaCtrl := emulator.NewQuotaController(emulator.QuotaConfig{
		Enabled: true,
		Rules:   map[string]emulator.RateRule{"iam/CreateUser": {Rate: 1, Burst: 3}},
	}, tc)
...
func TestQuotaController_UpdateConfig(t *testing.T) {
	...
	qc := emulator.NewQuotaController(cfg, tc)

	// Replace config with higher-rate rules.
	newCfg := emulator.QuotaConfig{
		Enabled: true,
		Rules: map[string]emulator.RateRule{
