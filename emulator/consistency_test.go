...
func newConsistencyCtrl(t *testing.T, enabled bool, delay time.Duration, services []string, tc *emulator.TimeController) *emulator.ConsistencyController {
	t.Helper()
	cfg := emulator.ConsistencyConfig{
		Enabled:          enabled,
		PropagationDelay: delay,
		AffectedServices: services,
	}
	ctrl, err := emulator.NewConsistencyController(cfg, tc)
	require.NoError(t, err)
	return ctrl
}

// iamReqCtx returns a minimal RequestContext for consistency tests.
func iamReqCtx() *emulator.RequestContext {
...
func TestConsistencyController_ResourceKeyIsolation(t *testing.T) {
	...
	writeAlice := &emulator.AWSRequest{
		Service: "iam", Operation: "CreateUser",
		Params: map[string]string{"UserName": "alice"},
	}
	readBob := &emulator.AWSRequest{
		Service: "iam", Operation: "GetUser",
...
func TestConsistencyController_ReplayingContext_NoOp(t *testing.T) {
	...
	replayCtx := &emulator.RequestContext{
...
func TestConsistencyController_InvalidConfig_Error(t *testing.T) {
	...
	_, err := emulator.NewConsistencyController(cfg, tc)
	require.Error(t, err)
}
...
func TestConsistencyController_Integration_Returns409(t *testing.T) {
	...
	consistencyCtrl, err := emulator.NewConsistencyController(emulator.ConsistencyConfig{
		Enabled:          true,
	...
	srv := emulator.NewServer(*cfg, registry, store, state, tc, logger,
		emulator.ServerOptions{Consistency: consistencyCtrl})
...
func TestConsistencyController_UpdateConfig(t *testing.T) {
	...
	cc, err := emulator.NewConsistencyController(cfg, tc)
	require.NoError(t, err)

	// Disable consistency.
	newCfg := emulator.ConsistencyConfig{
		Enabled:          false,
		PropagationDelay: 0,
	...
	req := &emulator.AWSRequest{Service: "iam", Operation: "GetUser"}
