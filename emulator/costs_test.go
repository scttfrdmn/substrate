...
func TestCostController_KnownOperations(t *testing.T) {
	ctrl := emulator.NewCostController(emulator.CostConfig{Enabled: true})
	...
	for _, tc := range tests {
		req := &emulator.AWSRequest{Service: tc.service, Operation: tc.operation}
...
func TestCostController_Unknown_ReturnsZero(t *testing.T) {
	ctrl := emulator.NewCostController(emulator.CostConfig{Enabled: true})
	req := &emulator.AWSRequest{Service: "unknownservice", Operation: "UnknownOp"}
	assert.Equal(t, 0.0, ctrl.CostForRequest(req))
}

func TestCostController_Disabled_AlwaysZero(t *testing.T) {
	ctrl := emulator.NewCostController(emulator.CostConfig{Enabled: false})

	for _, svc := range []string{"s3", "dynamodb", "lambda", "iam"} {
		req := &emulator.AWSRequest{Service: svc, Operation: "PutObject"}
		assert.Equal(t, 0.0, ctrl.CostForRequest(req),
...
func TestCostController_Override_WinsOverBuiltin(t *testing.T) {
	ctrl := emulator.NewCostController(emulator.CostConfig{
		Enabled: true,
	...
	req2 := &emulator.AWSRequest{Service: "s3", Operation: "GetObject"}
...
func TestGetCostSummary_AggregatesCorrectly(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		req := &emulator.AWSRequest{Service: tc.svc, Operation: tc.op}
...
func TestCostController_Integration_CostRecordedInStore(t *testing.T) {
	...
	costCtrl := emulator.NewCostController(emulator.CostConfig{Enabled: true})
...
func TestCostController_UpdateConfig(t *testing.T) {
	cc := emulator.NewCostController(emulator.CostConfig{Enabled: true})

	// Update with a custom cost override.
	newCfg := emulator.CostConfig{
		Enabled:   true,
