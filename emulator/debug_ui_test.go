...
func newDebugServer(t *testing.T) (*emulator.Server, *emulator.EventStore) {
	t.Helper()
	...
	if err := emulator.RegisterDefaultPlugins(ctx, registry, state, tc, logger, store, nil); err != nil {
...
func TestDebugEvents_WithEvents(t *testing.T) {
	...
	ctx := context.Background()
	if err := store.RecordEvent(ctx, &emulator.Event{
		Service:   "s3",
		Operation: "PutObject",
...
func TestDebugEvents_ServiceFilter(t *testing.T) {
	...
	for _, svc := range []string{"s3", "s3", "sqs"} {
		if err := store.RecordEvent(ctx, &emulator.Event{Service: svc, Operation: "op"}); err != nil {
			t.Fatal(err)
		}
	}
