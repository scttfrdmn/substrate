...
func newObsDeployer(t *testing.T) *emulator.StackDeployer {
	t.Helper()
	...
	ebPlugin := &emulator.EventBridgePlugin{}
	require.NoError(t, ebPlugin.Initialize(context.Background(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
		Options: map[string]any{"time_controller": tc},
	}))
	...
	cwPlugin := &emulator.CloudWatchPlugin{}
	require.NoError(t, cwPlugin.Initialize(context.Background(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
		Options: map[string]any{"time_controller": tc},
	}))
