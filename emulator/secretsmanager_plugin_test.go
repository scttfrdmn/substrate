...
func newSMTestServer(t *testing.T) *emulator.Server {
	t.Helper()
	...
	require.NoError(t, smPlugin.Initialize(context.TODO(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
...
func smRequest(t *testing.T, srv *emulator.Server, operation string, body interface{}) *http.Response {
	t.Helper()
