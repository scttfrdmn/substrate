...
func TestStringOrSlice_MarshalMultiple(t *testing.T) {
	s := emulator.StringOrSlice{"s3:GetObject", "s3:PutObject"}
	b, err := json.Marshal(s)
	require.NoError(t, err)
...
func TestPolicyPrincipal_UnmarshalWildcard(t *testing.T) {
	var p emulator.PolicyPrincipal
...
func TestPolicyPrincipal_MarshalWildcard(t *testing.T) {
	p := emulator.PolicyPrincipal{All: true}
...
func TestPolicyPrincipal_MarshalSingleAWS(t *testing.T) {
	p := emulator.PolicyPrincipal{AWS: []string{"arn:aws:iam::123456789012:root"}}
	b, err := json.Marshal(&p)
	require.NoError(t, err)
...
func TestPolicyDocument_RoundTrip(t *testing.T) {
		...
		Statement: []emulator.PolicyStatement{
			{
				...
				Resource: emulator.StringOrSlice{"arn:aws:s3:::mybucket/*"},
			},
		},
	}
	...
	var got emulator.PolicyDocument
	require.NoError(t, json.Unmarshal(b, &got))
	assert.Equal(t, doc.Version, got.Version)
	assert.Len(t, got.Statement, 1)
	assert.Equal(t, emulator.IAMEffectAllow, got.Statement[0].Effect)
}
...
func TestIAMARNHelpers(t *testing.T) {
	...
	tests := []struct {
		...
		{"userARN default path", emulator.IAMUserARNForTest(account, "", "alice"), "arn:aws:iam::123456789012:user/alice"},
		{"userARN custom path", emulator.IAMUserARNForTest(account, "/eng/", "bob"), "arn:aws:iam::123456789012:user/eng/bob"},
		{"roleARN", emulator.IAMRoleARNForTest(account, "", "myrole"), "arn:aws:iam::123456789012:role/myrole"},
		{"groupARN", emulator.IAMGroupARNForTest(account, "", "devs"), "arn:aws:iam::123456789012:group/devs"},
		{"policyARN", emulator.IAMPolicyARNForTest(account, "", "mypolicy"), "arn:aws:iam::123456789012:policy/mypolicy"},
	}
	for _, tt := range tests {
