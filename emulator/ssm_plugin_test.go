...
func newSSMTestServer(t *testing.T) *emulator.Server {
	t.Helper()
	...
	require.NoError(t, ssmPlugin.Initialize(context.TODO(), emulator.PluginConfig{
		State:   state,
		Logger:  logger,
...
func ssmRequest(t *testing.T, srv *emulator.Server, operation string, body interface{}) *http.Response {
	t.Helper()
