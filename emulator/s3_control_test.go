...
func newS3PresignTestServer(t *testing.T) *emulator.Server {
	t.Helper()
...
func TestCheckPresignedExpiry_NotPresigned(t *testing.T) {
	q := make(url.Values)
	assert.False(t, emulator.CheckPresignedExpiryForTest(q, time.Now()))
}
...
func TestCheckPresignedExpiry_NotExpired(t *testing.T) {
	...
	assert.False(t, emulator.CheckPresignedExpiryForTest(q, now))
}
...
func TestCheckPresignedExpiry_Expired(t *testing.T) {
	...
	q.Set("X-Amz-Date", "20260101T120000Z")
	q.Set("X-Amz-Expires", "3600")
	assert.True(t, emulator.CheckPresignedExpiryForTest(q, now))
}
...
func TestServer_PresignedURL_ExpiredReturns403(t *testing.T) {
	...
	tc := emulator.NewTimeController(future)
	logger := emulator.NewDefaultLogger(slog.LevelError, false)
...
func TestServer_PresignedURL_ValidPassesThrough(t *testing.T) {
	...
	tc := emulator.NewTimeController(issued)
	logger := emulator.NewDefaultLogger(slog.LevelError, false)
	srv := emulator.NewServer(*cfg, registry, store, state, tc, logger)
