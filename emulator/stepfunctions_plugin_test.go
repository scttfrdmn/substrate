...
func setupStepFunctionsPlugin(t *testing.T) (*emulator.StepFunctionsPlugin, *emulator.RequestContext) {
	t.Helper()
...
func sfnRequest(operation string, body map[string]any) *emulator.AWSRequest {
...
func sfnBody(t *testing.T, resp *emulator.AWSResponse) map[string]any {
	t.Helper()
...
func sfnCreate(t *testing.T, p *emulator.StepFunctionsPlugin, ctx *emulator.RequestContext, name, def, smType string) string {
	t.Helper()
	_, err := p.HandleRequest(ctx, sfnRequest("CreateStateMachine", map[string]any{
		"name":       name,
		"definition": def,
...
func sfnStartAndDescribe(t *testing.T, p *emulator.StepFunctionsPlugin, ctx *emulator.RequestContext, smArn, execName, input string) map[string]any {
	t.Helper()
...
func TestSFN_WaitState(t *testing.T) {
	p, ctx := setupStepFunctionsPlugin(t)
	tc := emulator.NewTimeController(time.Now())
	_ = p.Initialize(context.Background(), emulator.PluginConfig{
		State:   emulator.NewMemoryStateManager(),
...
func TestSFN_TaskState_Lambda(t *testing.T) {
	...
	sfnPlugin := &emulator.StepFunctionsPlugin{}
	require.NoError(t, sfnPlugin.Initialize(context.Background(), emulator.PluginConfig{
		State:  state,
		Logger: logger,
		Options: map[string]any{
			"time_controller": tc,
	...
	_, err := lambdaPlugin.HandleRequest(ctx, &emulator.AWSRequest{
		Service:   "lambda",
		Operation: "POST",
...
func TestSFN_RetryExhausted(t *testing.T) {
	...
	ctx2 := &emulator.RequestContext{AccountID: "123456789012", Region: "us-east-1"}
