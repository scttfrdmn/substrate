...
func setupECSPlugin(t *testing.T) (*emulator.ECSPlugin, *emulator.RequestContext) {
	t.Helper()
	...
	if err := p.Initialize(context.Background(), emulator.PluginConfig{
		...
		t.Fatalf("emulator.ECSPlugin.Initialize: %v", err)
	}
...
func ecsRequest(t *testing.T, op string, body map[string]any) *emulator.AWSRequest {
	t.Helper()
