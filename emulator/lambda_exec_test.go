...
func TestRuntimeToImage(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.runtime, func(t *testing.T) {
			t.Parallel()
			got := emulator.RuntimeToImage(tc.runtime)
			if got != tc.wantPfx {
...
func TestLambdaExecutor_IsDockerAvailable(t *testing.T) {
	...
	exec := emulator.NewLambdaExecutor(emulator.LambdaExecCfg{
		ReplayMode:  "live",
		WarmPoolTTL: 5 * time.Minute,
	}, logger)
	defer exec.StopAll()
...
func TestLambdaPlugin_Shutdown(t *testing.T) {
	...
	p := emulator.NewLambdaPluginForTest(state, tc)
	if err := emulator.ShutdownLambdaPluginForTest(p, t.Context()); err != nil {
...
func TestInvoke_DockerFallback(t *testing.T) {
	...
	defer exec.StopAll()
	emulator.LambdaPluginSetExecutorForTest(p, exec)
	...
	emulator.LambdaPluginCreateFunctionForTest(p, emulator.LambdaFunction{
		FunctionName: fnName,
		FunctionArn:  fnARN,
		Runtime:      "python3.12",
		Handler:      "index.handler",
	...
	resp, err := emulator.InvokeLambdaForTest(p, reqCtx, req, fnName)
	if err != nil {
