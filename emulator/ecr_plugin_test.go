...
func setupECRPlugin(t *testing.T) (*emulator.ECRPlugin, *emulator.RequestContext) {
	t.Helper()
	...
	if err := p.Initialize(context.Background(), emulator.PluginConfig{
		...
		t.Fatalf("emulator.ECRPlugin.Initialize: %v", err)
	}
...
func ecrRequest(t *testing.T, op string, body map[string]any) *emulator.AWSRequest {
	t.Helper()
...
func TestECRPlugin_CreateAndDescribeRepository(t *testing.T) {
	...
	awsErrOk := false
	if e, ok := err.(*emulator.AWSError); ok {
		awsErr = e
		awsErrOk = true
	}
