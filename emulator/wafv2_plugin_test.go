...
func setupWAFv2Plugin(t *testing.T) (*emulator.WAFv2Plugin, *emulator.RequestContext) {
	t.Helper()
...
func wafv2Request(t *testing.T, op string, body map[string]any) *emulator.AWSRequest {
	t.Helper()
...
func TestWAFv2Plugin_CreateGetUpdateDeleteWebACL(t *testing.T) {
	...
	var getResult struct {
		WebACL    emulator.WAFv2WebACL `json:"WebACL"`
		LockToken string               `json:"LockToken"`
	}
...
func TestWAFv2Plugin_IPSet_CRUD(t *testing.T) {
	...
	var getResult struct {
		IPSet     emulator.WAFv2IPSet `json:"IPSet"`
		LockToken string              `json:"LockToken"`
	}
	if err := json.Unmarshal(resp.Body, &getResult); err != nil {
		t.Fatalf("unmarshal get: %v", err)
...
//
// This 404 deliberately did not move with #1098's fifteen: the code belongs to the JSON protocol's
// Common Errors page, which publishes it at 404, not to WAFv2's own vocabulary — so the argument that
// moved WAFNonexistentItemException to 400 does not reach it. #1098's acceptance criteria assumed
// otherwise.
