...
func TestParseTerraformPlan_Valid(t *testing.T) {
	plan, err := emulator.ParseTerraformPlan(strings.NewReader(tfPlanFixture))
	if err != nil {
...
func TestParseTerraformPlan_Invalid(t *testing.T) {
	_, err := emulator.ParseTerraformPlan(strings.NewReader("not json"))
	if err == nil {
...
func TestValidateTerraformPlan_CostEstimate(t *testing.T) {
	...
	result, err := emulator.ValidateTerraformPlan(context.Background(), plan, costs)
	if err != nil {
...
func TestValidateTerraformPlan_EmptyPlan(t *testing.T) {
	...
	plan, err := emulator.ParseTerraformPlan(strings.NewReader(emptyPlan))
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	result, err := emulator.ValidateTerraformPlan(context.Background(), plan, nil)
	if err != nil {
		t.Fatalf("validate: %v", err)
	}
...
func TestValidateTerraformPlan_DeletedResources(t *testing.T) {
	...
	plan, err := emulator.ParseTerraformPlan(strings.NewReader(planJSON))
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	result, err := emulator.ValidateTerraformPlan(context.Background(), plan, nil)
