...
func injectDailyCosts(t *testing.T, store *emulator.EventStore, service string, costPerDay float64, daysAgo int) {
	t.Helper()
	...
	for d := 0; d < daysAgo; d++ {
		...
		require.NoError(t, emulator.RecordEventAtTimeForTest(store, ev))
	}
}
...
func TestGetCostForecast_AnomalyDetection(t *testing.T) {
	...
	require.NoError(t, emulator.RecordEventAtTimeForTest(store, &emulator.Event{
		Timestamp: spikeDay,
		AccountID: "123456789012",
		Service:   "lambda",
		Operation: "Invoke",
...
func TestMeanFloat(t *testing.T) {
	assert.Equal(t, 0.0, emulator.MeanFloatForTest(nil))
	assert.InDelta(t, 2.0, emulator.MeanFloatForTest([]float64{1, 2, 3}), 0.001)
}

func TestStddevFloat(t *testing.T) {
	assert.Equal(t, 0.0, emulator.StddevFloatForTest([]float64{5}, 5))
	vals := []float64{2, 4, 4, 4, 5, 5, 7, 9}
	mean := emulator.MeanFloatForTest(vals)
	sd := emulator.StddevFloatForTest(vals, mean)
